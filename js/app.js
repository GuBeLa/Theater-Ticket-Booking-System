...
$(document).ready(function () {
    ...
    const hoursCount = (start, end, id) => {
        let hours = [];
        for (let index = start; index <= end; index++) {
            if(index % 2 === 0) {
                let isCheck = localStorage.getItem(id)?.includes(index) ? 'off' : '';
                hours.push(`<li class="booking ${isCheck}" data-hours="${index}" data-id="${id}">${index}:00</li>`)
            }
        }
        return hours.join('');
    }
    ...
    const loadWeekDay = () => {
        ...
        $.each(data, function () {
            $.each(this, function (name, value) {
                weekDay.push(`<span class="week" data-week=${name}>${name}</span>`)
            });
        });
        week.html(weekDay)
    }

    const filterSession = (week = 'monday') => {
                    ...
                    $.each(value, function(index, item) {
                        filterData.push(`
                            <article>
                                ...
                                <h2>${item?.title}</h2>
                                <ul>
                                    ${hoursCount(item?.start, item?.end, item?.id)}
                                </ul>
                            </article>
                        `);
                    })
        ...
        container.html(filterData)
    }

    $(document).on("click", ".week", function (e) {
        let current = $(this).data('week');
    ...
    $(document).on("click", ".booking", function(e) {
        ...
        if (localStorage.getItem(id)) {
            newHours = [localStorage.getItem(id)];
            newHours.push(hours);
            localStorage.setItem(id, [newHours]);
        } else {
            localStorage.setItem(id, [hours]);
        }
